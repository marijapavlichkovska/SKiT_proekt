...
const safeClick = async (t, selector, timeout = 15000) => {
    ...
    while (Date.now() - start < timeout) {
        ...
        if (exists && visible && rect.width > 0 && rect.height > 0) {
            try {
                await t.scrollIntoView(selector); // no offset object
                await t.click(selector);
                return; // success
            } catch (err) {
                // element still blocked — wait and retry
            }
        }
...
const safeClickContact = async (t, selector, timeout = 20000) => {
    ...
    while (Date.now() - start < timeout) {
        await removeAdsContact();          // remove overlays
        await t.wait(300);          // wait for DOM to settle
...
const removeAdsContact = ClientFunction(() => {
    ...
    // Remove divs that are likely overlays or modals with high z-index
...
const fixGoogleVignette = ClientFunction(() => {
    try {
        ...
        // If Google Vignette added a hash or redirected to its page
